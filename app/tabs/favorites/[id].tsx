...
export default function Screen() {
    ...
    const { data } = useLiveQuery(
        drizzleDb.select().from(schema.movies).where(eq(schema.movies.movie_id, numberId))
    );
        ...
        <Animated.View style={[styles.container]}>
            <Stack.Screen
                options={{
                    ...
                    // headerRight: () => (
                    //     <Pressable onPress={() => handleSaveMovieOnList(data[0]!, 1)}>
                    ...
                    headerBackground: () => (
                        <Animated.View style={[styles.header, headerAnimatedStyle]}>
                            <Text style={{
                                ...
                                fontWeight: 'bold',
                                color: '#fff'
                            }}>{formatHeaderTitle(data[0]?.title)}</Text>
                        </Animated.View>
                    )
                }}
            />
            <Animated.ScrollView onScroll={onScroll} style={{flex: 1}} showsVerticalScrollIndicator={false}
                ...
                <MovieHeader titleAnimatedStyle={titleAnimatedStyle} animatedImageStyle={imageAnimatedStyle} poster={data[0]!.poster} title={data[0]!.title} originalTitle={data[0]!.originalTitle} />
                <View style={{
                    ...
                    <MovieDescription movie={{
                        ...
                        title: data[0]!.title,
                        originalTitle: data[0]!.originalTitle,
                        genres: data[0].genres!.split(' - '),
                        productionCompanies: data[0].productionCompanies!.split(' - '),
